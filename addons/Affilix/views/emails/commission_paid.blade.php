...
<head>
    ...
    <style>
        ...
        .amount { font-size: 28px; font-weight: bold; color: #2563eb; margin: 16px 0; }
...
<div class="container">
    <div class="header">
        <h1>{{ __('Affilix::affiliation.emails.commission_paid_subject') }}</h1>
    </div>
    <div class="body">
        <p>{{ __('Bonjour') }} <strong>{{ $commission->affiliate->customer->firstname ?? $commission->affiliate->customer->name }}</strong>,</p>
        <p>{{ __('Affilix::affiliation.emails.commission_paid_intro') }}</p>
        <div class="amount">{{ number_format($commission->amount, 2) }} {{ setting('currency_symbol', '€') }}</div>
        <div class="detail">
            <p><strong>{{ __('Affilix::affiliation.admin.payment_reference') }} :</strong> {{ $commission->payment_reference ?? '—' }}</p>
            <p><strong>{{ __('Affilix::affiliation.payment_method') }} :</strong> {{ $commission->affiliate->payment_method }}</p>
            <p><strong>{{ __('Affilix::affiliation.date') }} :</strong> {{ $commission->paid_at->format('d/m/Y') }}</p>
        </div>
        <p>{{ __('Affilix::affiliation.emails.commission_paid_detail') }}</p>
    </div>
    <div class="footer">
